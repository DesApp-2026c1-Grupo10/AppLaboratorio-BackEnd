...
export async function getResumen(req, res) {
  try {
    ...
    const movimientos = await db.MovimientoStock.findAll({
      attributes: ['materialId', 'reactivoId', 'cantidad', 'tipoMovimiento', 'createdAt'],
      include: [
        { model: db.Material, as: 'material', attributes: ['name'] },
        { model: db.Reagent, as: 'reactivo', attributes: ['name'] },
      ],
    });
    ...
    const laboratorioCount = pedidos.reduce((acc, p) => {
      ...
      if (!acc[nombre]) acc[nombre] = { count: 0, alumnos: 0, id: p.laboratorioId };
      acc[nombre].count += 1;
    ...
    const reactivoCount = movimientos
      .filter((m) => m.tipoMovimiento === 'salida' && m.reactivo)
      .reduce((acc, m) => {
        acc[m.reactivo.name] = (acc[m.reactivo.name] || 0) + m.cantidad;
        return acc;
      }, {});
    const reactivosMasUsados = Object.entries(reactivoCount)
      .map(([nombre, cantidad]) => ({ nombre, cantidad }))
      .sort((a, b) => b.cantidad - a.cantidad);
    ...
    const equiposSemana = usosEquipo.filter((u) => new Date(u.createdAt) >= inicioSemana);
    const movimientosSemana = movimientos.filter((m) => new Date(m.createdAt) >= inicioSemana);
