...
export async function getResumen(req, res) {
  try {
    ...
    const materialCount = movimientos
      .filter(
        (m) =>
          (m.tipoMovimiento === 'salida' || m.tipoMovimiento === 'usado') &&
          m.material
      )
      .reduce((acc, m) => {
    ...
    const materialDescarte = movimientos
      .filter((m) => m.tipoMovimiento === 'descarte' && m.material)
      .reduce((acc, m) => {
        acc[m.material.name] = (acc[m.material.name] || 0) + m.cantidad;
        return acc;
      }, {});
    const materialesDescartados = Object.entries(materialDescarte)
      .map(([nombre, cantidad]) => ({ nombre, cantidad }))
      .sort((a, b) => b.cantidad - a.cantidad);

    const reactivoDescarte = movimientos
      .filter((m) => m.tipoMovimiento === 'descarte' && m.reactivo)
      .reduce((acc, m) => {
        acc[m.reactivo.name] = (acc[m.reactivo.name] || 0) + m.cantidad;
        return acc;
      }, {});
    const reactivosDescartados = Object.entries(reactivoDescarte)
      .map(([nombre, cantidad]) => ({ nombre, cantidad }))
      .sort((a, b) => b.cantidad - a.cantidad);
    ...
    res.json({
      data: {
        ...
        reactivosMasUsados,
        sustanciasMasUsadas,
        materialesDescartados,
        reactivosDescartados,
        totalDescarte: movimientos
          .filter((m) => m.tipoMovimiento === 'descarte')
