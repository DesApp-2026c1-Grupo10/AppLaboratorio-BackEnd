...
export const accept = async (req, res) => {
  ...
  try {
    ...
    // Sincronizar Carrito con items de despensa de la revisión
    const descActualizada = pedido.descripcion || '';
    const dMatch = descActualizada.match(/__DESPENSA__:\{.*\}/);
    if (dMatch) {
      try {
        const jsonStr = dMatch[0].replace('__DESPENSA__:', '');
        const despensaData = JSON.parse(jsonStr);
        const tienesDespensa =
          (despensaData.despensaMateriales || []).length > 0 ||
          (despensaData.despensaReactivos || []).length > 0;
        ...
        if (tienesDespensa) {
          ...
          if (!carrito) {
            ...
            await carrito.update({ preparado: false }, { transaction: t });
          }

          const itemsCarrito = [];
          for (const dm of despensaData.despensaMateriales || []) {
          ...
          for (const dr of despensaData.despensaReactivos || []) {
        ...
        console.error('Error sincronizando carrito desde revisión:', e);
      }
    }
