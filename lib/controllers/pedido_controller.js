...
export const getAll = async (req, res) => {
  ...
  const where = {};
  if (rol === 'Profesor' && usuarioId) {
    where.usuarioId = Number(usuarioId);
  }
  const pedidos = await Pedido.findAll({ where, include: includeRecursos });
...
export const create = async (req, res) => {
  ...
  try {
    ...
    const conflictos = await Pedido.findAll({
      where: {
        fecha,
        laboratorioId,
        estado: { [Op.notIn]: ['Rechazado', 'Finalizado', 'Cancelado'] },
        horaInicio: { [Op.lt]: finBuffer },
        horaFin: { [Op.gt]: inicioBuffer },
      },
    });
    const aprobado = conflictos.find((p) => p.estado === 'Aprobado');
    const pendiente = conflictos.find((p) => p.estado === 'Pendiente');
    if (aprobado) {
      console.log('--- CREATE CONFLICTO BLOQUEANTE (APROBADO) ---');
      console.log(
        'Fecha:',
        fecha,
        'Lab:',
      ...
      throw new Error(
        'El laboratorio ya tiene un pedido APROBADO en ese horario. No se puede crear otro.'
      );
    }
    if (pendiente) {
      console.log('--- CREATE CONFLICTO DETECTADO (PENDIENTE) ---');
      console.log(
        'Fecha:',
        fecha,
        'Lab:',
      ...
      warnings.push(
        'El laboratorio tiene conflicto horario con otro pedido pendiente.'
      );
    }
    ...
    for (const eqId of equipos) {
      ...
      if (eqConflicto) {
        console.log('--- CREATE EQUIPO CONFLICTO ---');
        console.log('Equipo:', eq.name, 'ID:', eqId);
        console.log(
          'Conflicto con pedido ID:',
          eqConflicto.Pedido?.id,
          'Estado:',
          eqConflicto.Pedido?.estado
        );
        warnings.push(
    ...
    // 6. Crear pedido
    let despensaData = '';
    if (despensaMateriales.length > 0 || despensaReactivos.length > 0) {
    ...
    if (despensaMateriales.length > 0 || despensaReactivos.length > 0) {
      console.log('--- CREANDO CARRITO DESPENSA ---');
      console.log('Pedido ID:', pedido.id);
      console.log('Despensa materiales:', despensaMateriales);
      console.log('Despensa reactivos:', despensaReactivos);
      const carritoDespensa = await Carrito.create(
        { pedidoId: pedido.id, preparado: false },
        { transaction: t }
      );
      console.log('Carrito creado ID:', carritoDespensa.id);
      const itemsCarrito = [];
      ...
      if (itemsCarrito.length > 0) {
        ...
        console.log('Items carrito creados:', itemsCarrito.length);
      }
    }
...
export const check = async (req, res) => {
  ...
  try {
    ...
    const pendiente = conflictos.find((p) => p.estado === 'Pendiente');
    if (aprobado) {
      console.log('--- CHECK CONFLICTO BLOQUEANTE (APROBADO) ---');
      console.log(
        'Fecha:',
        fecha,
        'Lab:',
    ...
    if (pendiente) {
      console.log('--- CHECK CONFLICTO DETECTADO (PENDIENTE) ---');
      console.log('Fecha:', fecha, 'Lab:', laboratorioId);
      console.log(
        'Conflicto con pedido ID:',
        pendiente.id,
        'Estado:',
      ...
      warnings.push(
        'Ya hay un pedido PENDIENTE para este laboratorio en este horario'
      );
    }
...
export const update = async (req, res) => {
  ...
  try {
    ...
    // Remover `materiales`/`reactivos`/`equipos` del body antes de update para no pasarlos a Sequelize
    const {
      materiales: matsBody,
      reactivos: reasBody,
      equipos: eqsBody,
    ...
    await pedido.update(bodySinRelaciones, { transaction: t });

    // Actualizar materiales asociados
    if (matsBody !== undefined) {
      await PedidoMaterial.destroy({
        where: { pedidoId: pedido.id },
        transaction: t,
      });
      for (const m of matsBody) {
        await PedidoMaterial.create(
          { pedidoId: pedido.id, materialId: m.id, cantidad: m.cantidad || 1 },
          { transaction: t }
        );
      }
      cambios['materiales'] = {
        antes: 'actualizado',
        despues: `${matsBody.length} items`,
      };
    }

    // Actualizar reactivos asociados
    if (reasBody !== undefined) {
      await PedidoReactivo.destroy({
        where: { pedidoId: pedido.id },
        transaction: t,
      });
      for (const r of reasBody) {
        await PedidoReactivo.create(
          { pedidoId: pedido.id, reagentId: r.id, cantidad: r.cantidad || 1 },
          { transaction: t }
        );
      }
      cambios['reactivos'] = {
        antes: 'actualizado',
        despues: `${reasBody.length} items`,
      };
    }

    // Actualizar equipos asociados
    if (eqsBody !== undefined) {
      await PedidoEquipo.destroy({
        where: { pedidoId: pedido.id },
        transaction: t,
      });
      for (const eqId of eqsBody) {
        await PedidoEquipo.create(
          { pedidoId: pedido.id, equipmentId: eqId },
          { transaction: t }
        );
      }
      cambios['equipos'] = {
        antes: 'actualizado',
        despues: `${eqsBody.length} items`,
      };
    }
  ...
  try {
    ...
    for (const eq of equipos) {
      if (eq.is_movable && eq.laboratorioId !== pedido.laboratorioId) {
        // Guardar ubicación original antes de mover
        const pe = await PedidoEquipo.findOne({
          where: { pedidoId: pedido.id, equipmentId: eq.id },
          transaction: t,
        });
        if (pe) {
          await pe.update(
            { laboratorioIdOriginal: eq.laboratorioId },
            { transaction: t }
          );
        }
        await eq.update(
    ...
    // Verificar y sincronizar carrito de despensa
    ...
    if (despensaMatchAprob) {
      let despensaData;
      try {
        despensaData = JSON.parse(despensaMatchAprob[1]);
      } catch {
        throw new Error('Error parseando datos de despensa');
      }
      const despensaMateriales = despensaData.despensaMateriales || [];
      const despensaReactivos = despensaData.despensaReactivos || [];

      if (despensaMateriales.length > 0 || despensaReactivos.length > 0) {
        let carritoDespensa = await Carrito.findOne({
          where: { pedidoId: pedido.id },
        ...
        if (!carritoDespensa) {
          carritoDespensa = await Carrito.create(
            { pedidoId: pedido.id, preparado: false },
            { transaction: t }
          );
        }

        const itemsExistentes = carritoDespensa.items || [];

        // Eliminar items que ya no están en __DESPENSA__
        for (const item of itemsExistentes) {
          const existeEnDespensa =
            (item.tipo === 'material' &&
              despensaMateriales.some((d) => d.id === item.itemId)) ||
            (item.tipo === 'reactivo' &&
              despensaReactivos.some((d) => d.id === item.itemId));
          if (!existeEnDespensa) {
            await item.destroy({ transaction: t });
          }
        }

        // Agregar items nuevos que no están en el carrito
        let huboCambios = false;
        for (const dm of despensaMateriales) {
          const yaExiste = itemsExistentes.some(
            (i) => i.tipo === 'material' && i.itemId === dm.id
          );
          if (!yaExiste) {
            huboCambios = true;
            const mat = await Material.findByPk(dm.id, {
        ...
        for (const dr of despensaReactivos) {
          const yaExiste = itemsExistentes.some(
            (i) => i.tipo === 'reactivo' && i.itemId === dr.id
          );
          if (!yaExiste) {
            huboCambios = true;
            const rea = await Reagent.findByPk(dr.id, {
        ...
        if (huboCambios) {
          await carritoDespensa.update(
            { preparado: false },
            { transaction: t }
          );
        }

        // Verificar si el carrito está preparado
        const itemsActualizados = await CarritoItem.findAll({
          where: { carritoId: carritoDespensa.id },
          transaction: t,
        });
        if (itemsActualizados.length > 0 && !carritoDespensa.preparado) {
          throw new Error(
...
export const rechazar = async (req, res) => {
  ...
  try {
    ...
    // Devolver equipos (solo liberar estado, no cambiar ubicacion)
...
export const cancelar = async (req, res) => {
  ...
  try {
    ...
    // Liberar equipos (solo liberar estado, no cambiar ubicacion)
    const equipos = await pedido.getEquipments();
    for (const eq of equipos) {
      await eq.update({ status: 'Disponible' }, { transaction: t });
    }
...
export const finalizar = async (req, res) => {
  ...
  try {
    ...
    // 2b. Procesar items de despensa (vienen en el body pero no en pedido.materiales/reactivos)
    const matsIdsEnPedido = new Set(pedido.materiales.map((m) => m.id));
    const reasIdsEnPedido = new Set(pedido.reactivos.map((r) => r.id));
    for (const mItem of materialesFinal || []) {
      if (matsIdsEnPedido.has(mItem.id)) continue;
      const mat = await Material.findByPk(mItem.id, { transaction: t });
      if (!mat) continue;
      const esDescarte = materialesDescarte.includes(mat.id);
      const cantidad = mItem.cantidad ?? 1;
      if (esDescarte) {
        await mat.update({ stock: mat.stock - cantidad }, { transaction: t });
      }
      await MovimientoStock.create(
        {
          tipoMovimiento: esDescarte ? 'descarte' : 'usado',
          cantidad,
          fecha: new Date().toISOString().split('T')[0],
          observacion: `Consumo (despensa) tras pedido #${pedido.id}`,
          usuarioId,
          materialId: mat.id,
        },
        { transaction: t }
    ...
    for (const rItem of reactivosFinal || []) {
      if (reasIdsEnPedido.has(rItem.id)) continue;
      const rea = await Reagent.findByPk(rItem.id, { transaction: t });
      if (!rea) continue;
      const esDescarte = reactivosDescarte.includes(rea.id);
      const cantidad = rItem.cantidad ?? 1;
      if (esDescarte) {
    ...
    for (const eq of equipos) {
      ...
      if (estadoFinal === 'Disponible') {
        ...
        if (pe && pe.laboratorioIdOriginal != null) {
          updateData.laboratorioId = pe.laboratorioIdOriginal;
        }
      }
